...
#ifdef ARDUINO_ARCH_SAMD              // __SAMD21__ for more specific guard
  NVIC_SetPriority(SysTick_IRQn, 2);  // Give SysTick a high priority
  NVIC_SetPriority(EIC_IRQn, 3);      // Lower priority for external interrupt
#pragma "Arduino SAMD Variation Implemented"
#endif
}
...
void rotaryEncodeSensor::printPosStats() {
  ...
  sprintf(buffer, "time: %s -> encode: %5i, %s deg, %s rad", timeString, encoderPos, degString, radString);  // Do it this way to keep coloumns of data in line.
...
/*
#ifdef __SAMD21__ || ARDUINO_ARCH_SAMD
__disable_irq(); // ARM Cortex-M0+ specific method
#endif
__enable_irq(); // ARM Cortex-M0+ specific method
*/
...
void rotaryEncodeSensor::_isr_B() {
  noInterrupts();
  B_state = digitalRead(interruptPinB);
  // previousSampleTime_uS = currentSampleTime_uS;   // dont need to sample time on isr_B
  // currentSampleTime_uS = micros();
  encoderLastPos = encoderPos;
  // could below be done outside of ISR?
  encoderPos += (A_state == B_state) ? +1 : -1;
  rotaryEncodeSensor::encoderWrap();
  // ISR_triggered = true;    // dont think this is needed for signal analysis here, just on isrA
  interrupts();
}
